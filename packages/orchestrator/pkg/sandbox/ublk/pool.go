...
// New creates a ublk device, freeing the pool slot if creation fails.
...
// Close frees the device's pool slot; safe to call more than once for the same device.
...
// Shutdown closes every still-open device in parallel on process exit.
