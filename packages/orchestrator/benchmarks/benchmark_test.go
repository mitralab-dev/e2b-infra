...
func BenchmarkBaseImageLaunch(b *testing.B) {
	...
	sandboxFactory := sandbox.NewFactory(b.Context(), config.BuilderConfig, networkPool, devicePool, nil, featureFlags, hoststats.NewNoopDelivery(), cgroup.NewNoopManager(), network.NewNoopEgressProxy(), sandbox.NoopNetworkAssignHook{}, sandboxes)
